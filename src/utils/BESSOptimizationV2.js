...
export const BESSOptimization = ( capacity, power, loadData, energyChargeData, inputs ) => {
  ...
  const calculateCostWithBattery = async ({E_cap_year}) => {
    // Load GLPK
    let glpk = await GLPK();
    ...
    for (let t = 0; t < T; t++) {
        ...
        bnds: { type: glpk.GLP_DB, lb: SoC_min * E_cap_year, ub: SoC_max * E_cap_year }
        });
    ...
    for (let t = 0; t < T; t++) {
      ...
      lp.subjectTo.push({
        ...
        bnds: { type: glpk.GLP_UP, ub: SoC_max * E_cap_year }
      });
      ...
      lp.subjectTo.push({
        ...
        bnds: { type: glpk.GLP_LO, lb: SoC_min * E_cap_year }
      });
    }
  ...
  const calculateNPV = async ({}) => {
    ...
    let annualResults, annualCostWithBattery, newLoad, energyProfile, SoCProfile;

    for (let year = 1; year <= holdingPeriod; year++) {
      let E_cap_year = E_cap * Math.pow(1 - degradationRate, year-1);
      try {
        // let annualResults = calculateCostWithBattery(E_cap_year);
        // let annualCostWithBattery = annualResults.optimalCost;
        if (year === 1) {
          annualResults = await calculateCostWithBattery(E_cap_year);
          annualCostWithBattery = annualResults.optimalCost;
          newLoad = annualResults.newLoad;
          energyProfile = annualResults.energyProfile;
          SoCProfile = annualResults.SoCProfile;
        }
        ...
        console.error('Error in calculateNPV:', error);
        // Handle error appropriately
      }
    }

    return { newLoad, energyProfile, SoCProfile, npvWithBattery, npvByYear };
  };

  const generalResults = calculateNPV();
  
  return {
    newLoad: generalResults.newLoad,
    energyProfile: generalResults.energyProfile,
    SoCProfile: generalResults.SoCProfile,
    npv: generalResults.npvWithBattery,
    npvByYear: generalResults.npvByYear,
    capacity: capacity,
    power: power
  };
};
